...
export const ThemeContextProvider: React.FC<ThemeContextProviderProps> = ({
    ...
    const [theme, setCurrentTheme] = useState<Theme>(localStorage.getItem('theme') as Theme || 'system')
    ...
    const value: ThemeContextProps = {
        theme,
        fullTheme: theme === 'system' ? (systemThemeIsDark ? 'dark' : 'light') : theme,
        setTheme,
    }
    ...
    useEffect(() => {
        if(window.matchMedia) {
            const handleSystemThemeChange = (event: MediaQueryListEvent) => {
                logger.debug(`System theme changed to ${event.matches ? 'dark' : 'light'}`)
